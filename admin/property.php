       ...
       bathroom_id='$bathrooms',
        ...
        if ($result) {
            if(!empty($extra)) {
                ...
                //     $check = $add;
                    $sql4 = "SELECT * FROM featured_amenities WHERE pfeature_id='$add' AND property_id='$pid'";
                    $sql4 = "UPDATE featured_amenities SET pfeature_id='$add' WHERE property_id= '$pid'";
            ...
            // $result4 = $this->con->query($sql4); 
            // $amenity_property=[];
            // foreach($result4 as $amenity_property){
    ...
    public function addJointVenture($userid, $staff, $title, $prodesc, $offer, $address, $joint, $state, $city, $extra, $images)
    {
        ...
        if ($result) {
            if ($jointventure_id) {
                ...
                }else{$output_dir = "images/sponsor/"; /* Path for file upload */}
                $fileCount = count($images['name']);
        ...
        if ($result) {
            if ($jointventure_id) {
                if (!empty($extra)) {
                    foreach ($extra as $add) {
                        $sql3 = "INSERT IGNORE INTO sponsorshipneed SET jointventure_id= '$jointventure_id', jointtype_id='$add'";
                        $this->con->query($sql3);
...
// Maximum number of links per page.  If exceeded, google style pagination is generated
    ...
    public function showJointProperties(){
        $sql = "SELECT * FROM joint_venture
        LEFT JOIN agents ON joint_venture.agent_id = agents.agent_id
        LEFT JOIN states ON joint_venture.states_id = states.states_id
        LEFT JOIN city ON joint_venture.city_id = city.city_id
