...
if( $nameErr==1 && $emailErr==1 && $phoneErr==1 && $msgErr==1)
{
   ...
   store_message($fullName,$emailAddress,$phone,$message);
...
function legal_input($value) {
  ...
  function send_mail($fullName,$emailAddress,$phone,$message){
              ...
              $message = '<h2>Contact Message From alongish.com contact us form</h2>
                        <h3>Full Name</h3>
                        <p>'.$fullName.'</p>
                        <h3>Email Address</h3>
                        <p>'.$emailAddress.'</p>
  ...
  function store_message($fullName,$emailAddress,$phone,$message){
  
     global $db;
     $sql="INSERT INTO contactform (fname,email,phone,message_content) VALUES('$fullName','$emailAddress','$phone','$message')";
    //  die($sql);
     $query=$db->query($sql);
      if($query)
      {
