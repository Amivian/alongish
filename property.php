...
function getSponsorship($selected=''){
...
if(($page >=1)&&($page!=$total_pages)){ 
 ...
 public function showAgentjointMessage($id){
        $sql = "SELECT * FROM jointventure_message JOIN agents ON jointventure_message.agent_id = agents.agent_id WHERE jointventure_message.agent_id ='$id' AND jointventure_message.jmstatus='approved' ORDER BY date_posted DESC limit 3";
        // die($sql);
        $result=$this->con->query($sql);
    	$data = [];
    	while($row = $result->fetch_assoc()) {
    ...
    public function showAgentContactMessage($id){
        $sql = "SELECT * FROM agent_message JOIN agents ON agent_message.agent_id = agents.agent_id WHERE agent_message.agent_id ='$id' ORDER BY date_posted DESC limit 3";
        // die($sql);
        $result=$this->con->query($sql);
    	$data = [];
    	while($row = $result->fetch_assoc()) {
    ...
    // public function salesProperties($search,$type, $city)
    // {
    ...
    //     }
    //     //$sql = "SELECT * FROM p_listing $where";

    //     $sql = "SELECT * FROM property JOIN states ON property.states_id = states.states_id   JOIN property_type ON property.ptype_id = property_type.ptype_id  JOIN bedroom ON bedroom.bedroom_id=property.bedroom_id JOIN bathroom ON bathroom.bathroom_id=property.bathroom_id JOIN agents ON property.agent_id = agents.agent_id  JOIN city ON property.city_id = city.city_id JOIN property_status ON property.pstatus_id=property_status.pstatus_id $where property.pstatus_id = '2' ORDER BY property_id DESC";
    ...
    if(isset($_GET['page']) ? $page = $_GET['page']:$page = 1);
    // set the number of entries to appear on the page
    $sql1 = mysqli_query($this->con,"SELECT COUNT(property_id) As total_records FROM property JOIN city ON property.city_id = city.city_id JOIN property_status ON property.pstatus_id=property_status.pstatus_id $where property.pstatus_id = '2' AND pstatus='approved' ");
    $total_records= mysqli_fetch_array($sql1);
    $total_records = $total_records['total_records'];
    $entries_per_page = 10;  
    // total pages is rounded up to nearest integer
    ...
    // offset is used by SQL query in the LIMIT
    $offset = (($page * $entries_per_page) - $entries_per_page); 
    $sql="SELECT * FROM property JOIN city ON property.city_id = city.city_id JOIN property_type ON property.ptype_id = property_type.ptype_id  JOIN bedroom ON bedroom.bedroom_id=property.bedroom_id JOIN bathroom ON bathroom.bathroom_id=property.bathroom_id JOIN agents ON property.agent_id = agents.agent_id  JOIN states ON property.states_id = states.states_id JOIN property_status ON property.pstatus_id=property_status.pstatus_id $where property.pstatus_id = '2' AND property.pstatus='approved' ORDER BY property_id DESC LIMIT $offset, $entries_per_page";
    // die($sql);
    $result= $this->con->query($sql);
    $data =[];
        while($row = $result->fetch_array()) {
    ...
    //     $sql = "SELECT * FROM property JOIN city ON property.city_id = city.city_id JOIN property_type ON property.ptype_id = property_type.ptype_id  JOIN bedroom ON bedroom.bedroom_id=property.bedroom_id JOIN bathroom ON bathroom.bathroom_id=property.bathroom_id JOIN agents ON property.agent_id = agents.agent_id  JOIN states ON property.states_id = states.states_id JOIN property_status ON property.pstatus_id=property_status.pstatus_id $where property.pstatus_id = '1' AND property.pstatus='approved' ORDER BY property_id DESC";
    //     // die($sql);
    //     $result = $this->con->query($sql);
...
if(isset($_GET['page']) ? $page = $_GET['page']:$page = 1);
// set the number of entries to appear on the page
$sql1 = mysqli_query($this->con,"SELECT COUNT(property_id) As total_records FROM property JOIN city ON property.city_id = city.city_id JOIN property_status ON property.pstatus_id=property_status.pstatus_id $where property.pstatus_id = '1' AND pstatus='approved' ");
$total_records= mysqli_fetch_array($sql1);
$total_records = $total_records['total_records'];
$entries_per_page = 10;  
// total pages is rounded up to nearest integer
...
// offset is used by SQL query in the LIMIT
$offset = (($page * $entries_per_page) - $entries_per_page); 
$sql="SELECT * FROM property JOIN city ON property.city_id = city.city_id JOIN property_type ON property.ptype_id = property_type.ptype_id  JOIN bedroom ON bedroom.bedroom_id=property.bedroom_id JOIN bathroom ON bathroom.bathroom_id=property.bathroom_id JOIN agents ON property.agent_id = agents.agent_id  JOIN states ON property.states_id = states.states_id JOIN property_status ON property.pstatus_id=property_status.pstatus_id $where property.pstatus_id = '1' AND property.pstatus='approved' ORDER BY property_id DESC LIMIT $offset, $entries_per_page";
// die($sql);
$result= $this->con->query($sql);
$data =[];
    while($row = $result->fetch_array()) {
...
if(isset($_GET['page']) ? $page = $_GET['page']:$page = 1);
// set the number of entries to appear on the page
$sql1 =  mysqli_query($this->con,"SELECT COUNT(jointventure_id) As total_records FROM joint_venture JOIN city ON joint_venture.city_id = city.city_id   $where  jstatus='approved' ");
$total_records= mysqli_fetch_array($sql1);
$total_records = $total_records['total_records'];
$entries_per_page = 10;  
// total pages is rounded up to nearest integer
...
// offset is used by SQL query in the LIMIT
$offset = (($page * $entries_per_page) - $entries_per_page); 
$sql="SELECT * FROM joint_venture JOIN city ON joint_venture.city_id = city.city_id    JOIN agents ON joint_venture.agent_id = agents.agent_id  JOIN states ON joint_venture.states_id = states.states_id $where  jstatus='approved' ORDER BY jointventure_id DESC LIMIT $offset, $entries_per_page";
// die($sql);
$result= $this->con->query($sql);
$data =[];
    while($row = $result->fetch_array()) {
...
if(isset($_GET['page']) ? $page = $_GET['page']:$page = 1);
// set the number of entries to appear on the page
$sql1 = mysqli_query($this->con,"SELECT COUNT(property_id) As total_records FROM property   JOIN city ON property.city_id = city.city_id JOIN property_status ON property.pstatus_id=property_status.pstatus_id $where property.pstatus_id = '3' AND pstatus='approved' ");
$total_records= mysqli_fetch_array($sql1);
$total_records = $total_records['total_records'];
$entries_per_page = 10;   
// total pages is rounded up to nearest integer
...
$offset = (($page * $entries_per_page) - $entries_per_page); 
        ...
        $sql = "SELECT * FROM property JOIN states ON property.states_id = states.states_id   JOIN property_type ON property.ptype_id = property_type.ptype_id  JOIN bedroom ON bedroom.bedroom_id=property.bedroom_id JOIN bathroom ON bathroom.bathroom_id=property.bathroom_id JOIN agents ON property.agent_id = agents.agent_id  JOIN city ON property.city_id = city.city_id JOIN property_status ON property.pstatus_id=property_status.pstatus_id $where property.pstatus_id = '3' AND pstatus='approved' ORDER BY date_posted DESC LIMIT $offset, $entries_per_page";
// die($sql);
$result= $this->con->query($sql);
$data =[];
    while($row = $result->fetch_array()) {
...
echo '<li class="page-item" disabled><a class="page-link" href="'.$webpage.'?page='.($page+1).'">Next</a></li>';
    ...
    // public function shortletProperties($search,$type, $city)
    // {
    ...
    //     $sql = "SELECT * FROM property JOIN states ON property.states_id = states.states_id   JOIN property_type ON property.ptype_id = property_type.ptype_id  JOIN bedroom ON bedroom.bedroom_id=property.bedroom_id JOIN bathroom ON bathroom.bathroom_id=property.bathroom_id JOIN agents ON property.agent_id = agents.agent_id  JOIN city ON property.city_id = city.city_id JOIN property_status ON property.pstatus_id=property_status.pstatus_id $where property.pstatus_id = '3' ORDER BY property_id DESC";
...
//  die($sql2);
 ...
 ////////////////////////////////////////
            ...
            //     $sql = "SELECT * FROM property LIMIT " . $this_page_firstresult . ',' . $result_per_page;
            //     $result = mysqli_query($con, $sql );
            //     while ($row = mysqli_fetch_array($result)){
            //         $row['id'] . ' ' .$row['property'] . '<br>';
            //     }
            //     for($page = 1; $page <=$number_of_pages; $page++){
            //         echo '<a href="properties.php?page =' . $page . '  ">' . $page . '</a>';
            //     }
    ...
    //     $sql= "SELECT * FROM property JOIN agents ON property.agent_id = agents.agent_id JOIN states ON property.states_id = states.states_id JOIN city ON property.city_id = city.city_id JOIN bathroom ON property.bathroom_id = bathroom.bathroom_id JOIN property_status ON property.pstatus_id = property_status.pstatus_id JOIN property_type ON property.ptype_id=property_type.ptype_id JOIN bedroom ON property.bedroom_id =bedroom.bedroom_id WHERE property.staff= 'staff'";        
    //     // die($sql);
    //     $result = $this->con->query($sql);
    //     $data =[];
    //     while($row = $result->fetch_assoc()) {
    ...
    if(isset($_GET['page']) ? $page = $_GET['page']:$page = 1);
    // set the number of entries to appear on the page
    $sql1 = mysqli_query($this->con,"SELECT COUNT(property_id) As total_records FROM property WHERE staff='staff' ");
    $total_records= mysqli_fetch_array($sql1);
    $total_records = $total_records['total_records'];
    $entries_per_page = 2;  
    // total pages is rounded up to nearest integer
    ...
    // offset is used by SQL query in the LIMIT
    $offset = (($page * $entries_per_page) - $entries_per_page); 
    $sql="SELECT * FROM property JOIN agents ON property.agent_id = agents.agent_id JOIN states ON property.states_id = states.states_id JOIN city ON property.city_id = city.city_id JOIN bathroom ON property.bathroom_id = bathroom.bathroom_id JOIN property_status ON property.pstatus_id = property_status.pstatus_id JOIN property_type ON property.ptype_id=property_type.ptype_id JOIN bedroom ON property.bedroom_id =bedroom.bedroom_id WHERE property.staff= 'staff' ORDER BY date_posted DESC LIMIT $offset, $entries_per_page";
    // die($sql);
    $result= $this->con->query($sql);
    $data =[];
        while($row = $result->fetch_array()) {
    ...
    public function showAdminSwap(){
        $sql= "SELECT * FROM swaps JOIN agents ON swaps.agent_id = agents.agent_id JOIN states ON swaps.states_id = states.states_id JOIN city ON swaps.city_id = city.city_id  WHERE swaps.staff= 'staff' order by date_posted desc limit 2";       
        // die($sql);
        $result = $this->con->query($sql);
        $data =[];
        while($row = $result->fetch_assoc()) {
    ...
    public function showTeamMembers(){
        $sql="SELECT * FROM team JOIN agents ON team.agent_id=agents.agent_id WHERE team.staff='staff'";
        // die($sql);
        $result = $this->con->query($sql);
        $data =[];
        while($row = $result->fetch_assoc()) {
    ...
    function store_message($fname,$email,$phone,$message,$id){
...
// public function getJointVenture($id){
//     $sql = "SELECT * FROM joint_venture JOIN agents ON joint_venture.agent_id = agents.agent_id JOIN states ON joint_venture.states_id = states.states_id JOIN city ON joint_venture.city_id = city.city_id   WHERE joint_venture.agent_id ='$id' ORDER BY date_posted DESC";
//     // die($sql);
//     $result=$this->con->query($sql);
...
//     $sql = "SELECT * FROM jointventure_message JOIN agents ON jointventure_message.agent_id = agents.agent_id WHERE jointventure_message.agent_id ='$id' AND jointventure_message.jmstatus='approved' ORDER BY date_posted DESC";
//     // die($sql);
//     $result=$this->con->query($sql);
...
public function showRecentJoint(){
    $sql= "SELECT * FROM joint_venture JOIN agents ON joint_venture.agent_id = agents.agent_id JOIN states ON joint_venture.states_id = states.states_id JOIN city ON joint_venture.city_id = city.city_id order by date_posted desc limit 3";
    $result = $this->con->query($sql);
    // die($sql);
    $data =[];
    while($row = $result->fetch_assoc()) {
