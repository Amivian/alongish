...
function legal_input($value) {
  ...
  function send_mail($fullName,$emailAddress,$phone,$message){
              ...
              $subject = ' Sponsorship Message was sent by ' .$fullName;
              $message = '<h2>Sponsorship Message From alongish.com Joint Venture form</h2>
                        <h3>Full Name</h3>
                        <p>'.$fullName.'</p>
                        <h3>Email Address</h3>
                        <p>'.$emailAddress.'</p>
  ...
  function store_message($fullName,$emailAddress,$phone,$message,$a_id){
     ...
     global $db;
     $sql="INSERT INTO jointventure_message (fname,email,phone,message_content,agent_id) VALUES('$fullName','$emailAddress','$phone','$message','$a_id')";
    //  die($sql);
     $query=$db->query($sql);
      if($query)
      {
      $msg='Your Message was sent Successfully, Will we get back to you.';
      header("Location:joint-venture.php?msg=".$msg);
      }
      else
      {
       $msg="message failed";
